...
export const uploader = (fileAccepted: string[]) => {
  ...
  const fileFilter = (
    ...
    if(!fileAccepted.includes(file.mimetype)) return cb(new Error('File format not accepted'))
    cb(null, true);
  };

  return multer({ storage: storage, fileFilter: fileFilter, limits: {fileSize: 1024 * 1024 * 2} });
};
